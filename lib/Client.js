...
var debug = require("debug")("noble.js:client");
...
superagent.parse["text/plain"] = function (body) {
    try {
        debug("attempting to parse text/plain content", body);
        return JSON.parse(body);
    } catch (e) {
        debug("parsing failed", e.stack || e.message);
        return body;
    }
};
...
function Client(url, params) {
    ...
    debug("creating a new client object", url, params);

    if (!url) throw new Error("API URL required");
    this.root = url.replace(/\/$/g, ""); // strip trailing slash

    debug("root URL determined", this.root);

    if (params) {
        if (params.client_id || params.client_secret) {
            debug("using client id/secret");
            this.client_id     = params.client_id;
            this.client_secret = params.client_secret;
        }

        if (params.auth) {
            debug("using predefined auth");
            this.auth = params.auth;

            if (this.auth.user_id) {
                debug("creating a short-hand me (user) object");
                this.me = this.user(this.auth.user_id);
            }
        } else if (params.access_token) {
            debug("using predefined access token");
...
Client.prototype.request = function (method, uri) {
    ...
    if (this.auth && this.auth.access_token) {
        ...
        debug("appending access token query param", token);
...
Client.prototype.login = function (username, password, callback) {
    ...
    debug("oauth authentication: grant type 'password'");
    debug("client", this.client_id, this.client_secret);
...
Client.prototype.refresh = function (callback) {
    ...
    debug("oauth authentication: grant type 'refresh_token'");
    ...
    debug("client", this.client_id, this.client_secret);
    debug("refresh token", this.auth.refresh_token);
