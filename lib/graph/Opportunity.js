...
/**
 * POST a user asset to an opp's stored assets collection
 * Swagger: /swagger-ui/#/noblehour/post_opportunities__opportunity_id__users__author_id__assets
 *
 * @param {String} author_id
 * @param {Object} [query]
 * @param {Function} callback
 *
 * _required_ queryParams:
 * @param {File} asset
 * @param {String} name
 * @param {Boolean} required
 *
 */
Opportunity.prototype.userAsset = function (userID, query, callback) {
    ...
    return this.client.request("post", this.uri() + "/users/" + userID + "/assets")
        .field("name", query.name)
        .field("author_id", query.author_id)
        .field("opportunity_asset_type", query.opportunity_asset_type)
        .field("opportunity_id", query.opportunity_id)
        .attach("asset", query.asset, query.asset.name)
        .end(utils.easy(callback));
};
