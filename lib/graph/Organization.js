 ...
 * @param {Function} callback
 */
Organization.prototype.publicActivities = function(query, callback) {
    if (typeof query === "function") {
        callback = query;
        query = null;
    }

    var req = this.client.request(this.uri() + "/activities/public");
 ...
 Organization.prototype.groups = function (query, callback) {
  ...
  /**
  * Retrieves a list of organization owner details
  *
  * Available `query` options:
  *  - limit {Number}
  *  - offset {Number}
  ...
  * @param {Function} callback
  */
 Organization.prototype.groupsDetailed = function (query, callback) {
     if (typeof query === "function") {
         callback = query;
         query = null;
     }

     return this.related("groups/detailed", query, callback);
 };
...
Organization.prototype.opportunities = function (query, callback) {
    ...
    return this.related("report/opportunities_with_contributed_hours", query, callback);
};
...
/**
  * Retrieves a list of host organizations for this organization
  * host orgs are organizations that host opportunities related to this organization
  *
  * Available `query` options:
  *  - limit {Number}
  *  - offset {Number}
...
Organization.prototype.hosts = function (query, callback) {
    ...
    return this.related("report/hosts_of_contributed_hours", query, callback);
};

/**
  * Submit an opportunity to be hosted by the organization
  * Typically done when an opp is created or when its host_id is edited
  *
  * Required `query` options:
  *  - opportunity_id {String} the ID of the opportunity to be submitted
  *
  * @param {oppID} string
  * @param {Function} callback
  */
Organization.prototype.submitOpp = function (oppID, callback) {
    return this.client.request("post", this.uri() + "/hosted")
      .send({
 ...
 /**
  * Retrieves a list of all tags for the organization
  *
  * Available `query` params:
  *  - category {Number}   The category describes the context for the drop down in the UI
  *  - search {String}     Return only those tags that start with this value
  *
  * @param {Object} [query]
  * @param {Function} callback
  * @returns {superagent.Request}
  */
 Organization.prototype.tags = function (query, callback) {
    if (typeof query === "function") {
        callback = query;
        query = null;
    }

    return this.related("tags", query, callback);
};
// single export
