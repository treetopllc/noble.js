...
/**
 * Retrieves a list of projects this user is affiliated with
 *
 * Available `query` options:
 *  - limit {Number}
 *  - offset {Number}
 ...
 * @param {Function} callback
 */
User.prototype.projects = function(query, callback) {
    if (typeof query === "function") {
        callback = query;
        query = null;
    }

    return this.related("projects", query, callback);
};
...
/**
 * Retrieves a user's opportunties with hours summary
 *
 * @param {String} id           UUID
 * @param {Object} params
 * @param {Function} callback
