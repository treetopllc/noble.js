...
describe("lib/graph/User.js", function() {
    describe("User", function() {
        ...
        describe("#incomingSubmissions([query], callback)", function() {
            it("should pass a smoke test", function(done) {
                server.respondWith("/users/abc/submissions/incoming", simpleResponse);

                user.incomingSubmissions(done);
            });
        });
        ...
        describe("#relationships([query], callback)", function() {
            it("should pass a smoke test", function(done) {
                server.respondWith("/users/abc/relationships", simpleResponse);

                user.relationships(done);
            });

            it("should pass additional query string arguments", function(done) {
                server.respondWith("/users/abc/relationships?limit=5", [
                    200,
                    defaultHeaders,
                    JSON.stringify(createArray(5, function() {
                        return {
        ...
        describe("#passwordUpdate(params, callback)", function() {
            it("should pass a smoke test", function(done) {
                server.respondWith("/users/abc", simpleResponse);

                user.passwordUpdate({}, done);
            });
        });
